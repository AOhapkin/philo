...
Проект "philo" - это проект в рамках обучения в школе 42, который требует написания программы для симуляции обеда философов и решения задачи о взаимной блокировке (deadlock) при использовании общих ресурсов.

Программа должна имитировать ситуацию, когда несколько философов сидят за круглым столом, каждый из которых имеет свою вилку. Философы могут находиться в одном из двух состояний - "думают" или "едят". Чтобы есть, философ должен держать две вилки - одну в правой, другую в левой руке. Одновременно можно держать только одну вилку, поэтому если философ справа и слева заняты вилками, то философ не может есть и должен ждать.

Цель проекта - написать программу, которая имитирует ситуацию с философами и решает проблему взаимной блокировки при использовании общих ресурсов. Программа должна быть многопоточной и работать на нескольких потоках. Каждый философ должен быть представлен отдельным потоком, а вилки - общими ресурсами, которые нужно защитить от одновременного доступа нескольких философов.
...
- [x] Тестовые значения `1 800 200 200`: философ не должен есть и должен умереть.
- [x] Тестовые значения `5 800 200 200`: никто не должен умереть.
- [x] Тестовые значения `5 800 200 200 7`: никто не должен умирать, симуляция остановится, когда каждый философ поест минимум 7 раз.
- [x] Тестовые значения `4 410 200 200`: никто не должен умереть.
- [x] Тестовые значения `4 310 200 100`: философ умрет.
